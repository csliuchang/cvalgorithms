...
def _create_gradient_clipper(cfg) -> _GradientClipper:
    ...
    return _GRADIENT_CLIP_TYPE_TO_CLIPPER[GradientClipType(cfg.type)]
...
def build_optimizer(cfg, model):
    ...
    for key, value in model.named_parameters(recurse=True):
        ...
        if "backbone" in key:
            lr = lr * cfg.backbone_multiplier
        params += [{"params": [value], "lr": lr, "weight_decay": weight_decay}]

    def maybe_add_full_model_gradient_clipping(optim):  # optim: the optimizer class
        # detectron2 doesn't have full model gradient clipping now
        clip_norm_val = cfg.clip_gradients.clip_value
        enable = (
                cfg.clip_gradients.enabled
                and cfg.clip_gradients.type == "full_model"
    ...
    if optimizer_type == "SGD":
        ...
        optimizer = maybe_add_full_model_gradient_clipping(torch.optim.AdamW)(
            params, cfg.learning_rate
        )
    else:
        raise NotImplementedError(f"no optimizer type {optimizer_type}")
    if not cfg.clip_gradients.clip_value == "full_model":
