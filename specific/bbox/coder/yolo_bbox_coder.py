...
class YOLOBBoxCoder(BaseBBoxCoder):
    ...
    ----------
    eps : float
        Min value of cx, cy when encoding.
    """
    ...
    def encode(self, bboxes, gt_bboxes, stride):
        ...
        x_center_target = ((x_center_gt - x_center) / stride.float() + 0.5).clamp(
            self.eps, 1 - self.eps)
        y_center_target = ((y_center_gt - y_center) / stride.float() + 0.5).clamp(
            self.eps, 1 - self.eps)
