...
class GridAssigner(BaseAssigner):
    ...
    min_pos_iou : float
        Minimum iou for a bbox to be considered as a
        positive bbox. Positive samples can have smaller IoU than
        pos_iou_thr due to the 4th step (assign max IoU sample to each gt).
    gt_max_assign_all : bool
        Whether to assign all bboxes with the same
    ...
    def assign(self, bboxes, box_responsible_flags, gt_bboxes, gt_labels=None):
        ...
        box_responsible_flags : Tensor
            flag to indicate whether box is
            responsible for prediction, shape(n, )
        gt_bboxes : Tensor
            Groundtruth boxes, shape (k, 4).
        ...
        overlaps[:, ~box_responsible_flags.byte()] = -1.

        max_overlaps, argmax_overlaps = overlaps.max(dim=0)

        gt_max_overlaps, gt_argmax_overlaps = overlaps.max(dim=1)

        # pos_inds = (max_overlaps > self.pos_iou_thr) & box_responsible_flags.byte()
        pos_inds = (max_overlaps > self.pos_iou_thr) * box_responsible_flags
