...
class CAREERCDDataset(BaseDataset):
    def __init__(self, img_g_path, img_n_path, *args, **kwargs):
        ...
        super(CAREERCDDataset, self).__init__(*args, **kwargs)

    def load_image(self, index):
        img_str = self.data_infos[index]['filename'].split('images/')[-1]
        img_g_str = osp.join(self.data_root, self.img_g_path, img_str)
        img_n_str = osp.join(self.data_root, self.img_n_path, img_str.split('__')[0] + '__template_' + img_str.split('__')[1])
