...
def flatten_binary_logits(logits, labels, ignore_label=None):
...
def flatten_probs(probs, labels, ignore_label=None):
...
def lovasz_hinge(logits,
    ...
    Args:
        ...
        avg_factor (int, optional): Average factor that is used to average
            ...
            Default: None.
        ignore_label (int | None): The label index to be ignored. Default: 255.
    Returns:
        torch.Tensor: The calculated loss.
    """
    if per_image:
        loss = [
            lovasz_hinge_flat(*flatten_binary_logits(
                logit.unsqueeze(0), label.unsqueeze(0), ignore_label))
        ...
        loss = lovasz_hinge_flat(
            *flatten_binary_logits(logits, labels, ignore_label))
    return loss
...
def lovasz_softmax(probs,
    ...
    if per_image:
        loss = [
            lovasz_softmax_flat(
                *flatten_probs(
                    prob.unsqueeze(0), label.unsqueeze(0), ignore_label),
                classes=classes,
        ...
        loss = lovasz_softmax_flat(
            *flatten_probs(probs, labels, ignore_label),
            classes=classes,
            class_weight=class_weight)
    return loss
