...
from deepcv2.common import reduce_value


class BaseChanger(nn.Module):
    """Base class for changers."""

    __metaclass__ = ABCMeta

    def __init__(self):
        super(BaseChanger, self).__init__()
    ...
    def _parse_metrics(metrics):
        ...
        metrics = {key: reduce_value(loss, average=True) for key, loss in metrics.items() if "loss" in key}
