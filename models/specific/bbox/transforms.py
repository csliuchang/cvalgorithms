...
def bbox_flip(bboxes, img_shape, direction='horizontal'):
    ...
    img_shape : tuple
        Image shape.
    direction : str
        Flip direction, options are "horizontal", "vertical",
...
def roi2bbox(rois):
    ...
    ----------
    rois : torch.Tensor
        RoIs with the shape (n, 5) where the first
...
def bbox2result(bboxes, labels, num_classes):
    ...
    labels : torch.Tensor | np.ndarray
        shape (n, )
    num_classes : int
        class number, including background class
...
def distance2bbox(points, distance, max_shape=None):
    ...
    points : Tensor
        Shape (n, 2), [x, y].
    distance : Tensor
        Distance from the given point to 4
...
def bbox2distance(points, bbox, max_dis=None, eps=0.1):
    ...
    points : Tensor
        Shape (n, 2), [x, y].
    bbox : Tensor
        Shape (n, 4), "xyxy" format
    max_dis : float
        Upper bound of the distance.
    eps : float
        a small value to ensure target < max_dis, instead <=
...
def bbox_rescale(bboxes, scale_factor=1.0):
    ...
    ----------
    bboxes : Tensor
        Shape (n, 4) for bboxes or (n, 5) for rois
    scale_factor : float
        rescale factor
