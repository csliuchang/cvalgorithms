...
def windows_partition(x, windows_size):
    """
    Args:
        x: (B, H, W, C)
        window_size (int): window size
    ...
    """
    B, H, W, C = x.shape
    x = x.view(B, H // windows_size, windows_size, W // windows_size, windows_size, C)
...
class SwinTransformerV2(nn.Module):
    ...
    def __init__(self, img_size=224, patch_size=4, in_channels=3, num_classes=None,
    ...
    def forward(self, x):
        feature_lists = self.forward_features(x)
        if self.num_classes is not None:
            x = self.norm(feature_lists[-1])  # B L C
