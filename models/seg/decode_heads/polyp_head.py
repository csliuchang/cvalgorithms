    ...
    def init_weights(self):
        ...
        for ly in self.children():
            if isinstance(ly, nn.Conv2d):
                ...
                if not ly.bias is None: nn.init.constant_(ly.bias, bias_value)

    def forward(self, inputs):
        if len(inputs) == 5:
            x0, x1, x2, x3, x4 = inputs[0], inputs[1], inputs[2], inputs[3], inputs[4]
        else:
        ...
        return prediction2_8

    def losses(self, seg_logit, seg_label):
        loss = dict()
        loss_P1 = self.loss_1(seg_logit[0], seg_label)
        loss_P2 = self.loss_2(seg_logit[1], seg_label)
        loss["loss"] = loss_P1 + loss_P2
        return loss
