...
class PairBaseDetector(nn.Module, metaclass=ABCMeta):
    ...
    def forward(self, inputs_n, inputs_g, return_metrics=False, **kwargs):
        """Calls either :func:`forward_train` or :func:`forward_infer` depending
        on whether ``return_metrics`` is ``True``.
        """
        if return_metrics:
            bboxes, metrics = self.forward_train(inputs_n, inputs_g, **kwargs)
